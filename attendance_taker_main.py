...
create_table_sql = f"CREATE TABLE IF NOT EXISTS {table_name} (name TEXT, time TEXT, date DATE, major TEXT, UNIQUE(name, date))"
...
class Face_Recognizer:
    ...
    def attendance(self, name):
        ...
        if morning_start <= current_time <= morning_end:
            cursor.execute("SELECT * FROM attendance WHERE name = ? AND time BETWEEN ? AND ?",(name, morning_start.strftime('%H:%M:%S'), morning_end.strftime('%H:%M:%S')))
            existing_entry = cursor.fetchone()
        elif afternoon_start <= current_time <= afternoon_end:
            cursor.execute("SELECT * FROM attendance WHERE name = ? AND time BETWEEN ? AND ?",(name, afternoon_start.strftime('%H:%M:%S'), afternoon_end.strftime('%H:%M:%S')))
            existing_entry = cursor.fetchone()
        if existing_entry:
            #print(f"{name} is already marked as present for {current_date}")
            self.ctl_system.dataWriteLCD("Already Recorded")
            self.ctl_system.moveServo()
        else:
            ...
            if major:
                ...
                cursor.execute("INSERT INTO attendance (name, time, date, major) VALUES (?, ?, ?, ?)",(name, current_time_str, current_date, major))
                conn.commit()

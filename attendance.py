                    ...
                    # 6.2.2 / Face cnt increase: 0->1, 0->2, ..., 1->2, ...
                        ...
                        for k in range(len(faces)):
                            ...
                            if min_distance < threshold:
                                ...
                                accuracy_score = max(threshold - min_distance, 0) / threshold * 100

                                nam = self.face_name_known_list[similar_person_num]
                                print(f"Recognized: {nam}, Accuracy: {accuracy_score:.2f}%")
                                self.attendance(nam)

                                img_rd = cv2.putText(img_rd, f"Accuracy: {accuracy_score:.2f}%",
                                                     self.current_frame_face_position_list[k], self.font, 0.6, (0, 255, 0), 1, cv2.LINE_AA)
                                
                            else:
                                logging.debug("Face Recognition Result: Unknown Person")
                                img_rd = cv2.putText(img_rd, "Accuracy: Low", self.current_frame_face_position_list[k], self.font, 0.6, (0, 0, 255), 1, cv2.LINE_AA)
